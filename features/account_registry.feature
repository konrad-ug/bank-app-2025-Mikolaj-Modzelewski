Feature: Account registry
  ...
  Scenario: User is able to update name of already created account
    Given I create an account using name: "jan", last name: "kowalski", pesel: "90010100001"
    When I update "name" of account with pesel: "90010100001" to "adam"
    Then Account with pesel "90010100001" has "name" equal to "adam"

  Scenario: Created account has all fields correctly set
    When I create an account using name: "maria", last name: "curie", pesel: "80010112345"
    Then Account with pesel "80010112345" has "name" equal to "maria"
    And Account with pesel "80010112345" has "surname" equal to "curie"
    And Account with pesel "80010112345" has "balance" equal to "0"
  ...
  Scenario: User creates an account and receives a transfer
    Given I create an account using name: "elon", last name: "musk", pesel: "77777777777"
    When I make an "incoming" transfer of "100" to account with pesel: "77777777777"
    Then Account with pesel "77777777777" has "balance" equal to "100"

  Scenario: User creates an account and makes an outgoing transfer
    Given I create an account using name: "bill", last name: "gates", pesel: "66666666666"
    And I make an "incoming" transfer of "500" to account with pesel: "66666666666"
    When I make an "outgoing" transfer of "200" to account with pesel: "66666666666"
